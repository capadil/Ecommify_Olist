...
CASE
    WHEN ((geolocation_lat IS NOT NULL) AND (geolocation_lng IS NOT NULL)) THEN (extensions.st_setsrid(extensions.st_makepoint((geolocation_lng)::double precision, (geolocation_lat)::double precision), 4326))::extensions.geography
...
CREATE INDEX idx_category_name_english_trgm ON ecommify.category_translation USING gin (product_category_name_english extensions.gin_trgm_ops);
...
CREATE INDEX idx_category_name_trgm ON ecommify.category_translation USING gin (product_category_name extensions.gin_trgm_ops);
...
CREATE INDEX idx_customers_city_trgm ON ecommify.customers USING gin (customer_city extensions.gin_trgm_ops) WHERE (customer_city IS NOT NULL);
...
CREATE INDEX idx_order_reviews_message_trgm ON ecommify.order_reviews USING gin (review_comment_message extensions.gin_trgm_ops) WHERE (review_comment_message IS NOT NULL);
...
CREATE INDEX idx_order_reviews_title_trgm ON ecommify.order_reviews USING gin (review_comment_title extensions.gin_trgm_ops) WHERE (review_comment_title IS NOT NULL);
...
CREATE INDEX idx_sellers_city_trgm ON ecommify.sellers USING gin (seller_city extensions.gin_trgm_ops) WHERE (seller_city IS NOT NULL);
