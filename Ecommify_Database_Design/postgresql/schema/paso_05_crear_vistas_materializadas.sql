...
-- Objetivo: alimentar tableros sin ejecutar consultas pesadas directamente sobre tablas OLTP.
-- Nota: se crean WITH NO DATA; poblarlas en el paso 07.
...
SELECT
    ...
    COALESCE(ct.product_category_name_english, ct.product_category_name, 'unknown') AS category_name,
...
FROM orders o
JOIN order_items oi ON oi.order_sk = o.order_sk
...
SELECT
    ...
    CASE
        ...
        WHEN COUNT(DISTINCT o.order_sk) >= 3 THEN 'repeat_customer'
        ELSE 'standard'
    END AS customer_segment
FROM customers c
LEFT JOIN orders o ON o.customer_sk = c.customer_sk
LEFT JOIN order_payments op ON op.order_sk = o.order_sk
LEFT JOIN order_reviews r ON r.order_sk = o.order_sk
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_mv_customer_segments
    ON mv_customer_segments (customer_sk);
...
SELECT
    ...
    AVG(r.review_score) AS avg_review_score
FROM sellers s
JOIN order_items oi ON oi.seller_sk = s.seller_sk
JOIN orders o ON o.order_sk = oi.order_sk
LEFT JOIN order_reviews r ON r.order_sk = o.order_sk
GROUP BY 1, 2, 3, 4
WITH NO DATA;

CREATE UNIQUE INDEX IF NOT EXISTS ux_mv_seller_performance_monthly
    ON mv_seller_performance_monthly (sales_month, seller_sk);
...
SELECT
    ...
    COALESCE(SUM(op.payment_value), 0) AS total_payment_value
FROM customers c
JOIN orders o ON o.customer_sk = c.customer_sk
LEFT JOIN order_payments op ON op.order_sk = o.order_sk
GROUP BY 1, 2, 3
WITH NO DATA;
